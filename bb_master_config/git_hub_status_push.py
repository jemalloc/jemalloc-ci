...
# Forked from the buildbot version to allow the status to be sent to the
# destination repo instead of the source repo.
...
class GitHubStatusPush(http.HttpStatusPushBase):
    ...
    def send(self, build):
        ...
        if self.destRepoOwner is not None:
            repoOwner = self.destRepoOwner
        if self.destRepoName is not None:
            repoName = self.destRepoName
